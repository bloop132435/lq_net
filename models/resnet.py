...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1, args=None,bits=8):
    ...
    if args is not None and hasattr(args, 'keyword'):
        return custom_conv(in_planes, out_planes, kernel_size=3, stride=stride,
                ...
                args=args,bits_activations=bits,bits_weights=bits)
    else:
        return nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                padding=dilation, groups=groups, bias=False, dilation=dilation)


def conv1x1(in_planes, out_planes, bits,stride=1, args=None, force_fp=False):
    """1x1 convolution"""
    if args is not None and hasattr(args, 'keyword'):
        return custom_conv(in_planes, out_planes, kernel_size=1, stride=stride, bias=False,
                args=args, force_fp=force_fp,bits_activations=bits,bits_weights=bits)
    else:
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes,bit_iter, stride=1, downsample=None, groups=1,
        ...
        self.conv1 = conv3x3(inplanes, inner_planes, stride, args=args,bits=next(bit_iter))
        self.bn1 = norm_layer(inner_planes, args=args)
        self.conv2 = conv3x3(inner_planes, planes, args=args,bits=next(bit_iter))
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        if self.bits is None:
            self.bits = [args.fm_bit for _ in range(2000)]
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False,bits=32):
        downsample = None
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            if self.args is not None and hasattr(self.args, 'keyword'):
                if 'real_skip' in self.args.keyword:
                    force_fp = True
            tmp_bits = next(self.bits_iter)
            downsample = nn.Sequential(
                    conv1x1(in_planes=self.inplanes, out_planes=planes * block.expansion, stride = stride, args=self.args, force_fp=force_fp,bits=tmp_bits),
                    norm(planes * block.expansion, args=self.args),
                    )

        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
            self.base_width, previous_dilation, norm, args=self.args,bit_iter=self.bits_iter))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
                base_width=self.base_width, dilation=self.dilation,
                norm_layer=norm, args=self.args,bit_iter=self.bits_iter))
